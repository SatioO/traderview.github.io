...
import { CheckCircle2, AlertCircle, Copy, X, RefreshCw, TrendingUp, Zap, Clock, ArrowRight } from 'lucide-react';
...
export const PremiumOrderStatus: React.FC<PremiumOrderStatusProps> = ({
  ...
  const getErrorSeverity = () => {
    ...
    return ['VALIDATION_ERROR', 'INSUFFICIENT_FUNDS'].includes(error.code || '') ? 'warning' : 'error';
  };
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-green-100 font-bold text-lg tracking-wide">ORDER FILLED</div>
            ...
            <div className="bg-black/30 border border-green-500/20 rounded-xl p-3 mb-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="text-green-400 text-xs font-medium uppercase tracking-wider">Symbol</div>
                  <div className="text-green-100 font-semibold text-sm">
                    {orderResponse.orderData.tradingsymbol}
                  </div>
                </div>
                <div>
                  <div className="text-green-400 text-xs font-medium uppercase tracking-wider">Quantity</div>
                  <div className="text-green-100 font-semibold text-sm">
                    {orderResponse.orderData.quantity.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div className="text-green-400 text-xs font-medium uppercase tracking-wider">Type</div>
                  <div className="text-green-100 font-semibold text-sm">
                    {orderResponse.orderData.order_type}
                  </div>
                </div>
                <div>
                  <div className="text-green-400 text-xs font-medium uppercase tracking-wider">Status</div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <div className="text-green-400 text-xs font-medium uppercase tracking-wider">Order ID</div>
  ...
  if (type === 'error' && error) {
    ...
    return (
      <div className="relative overflow-hidden">
        ...
        <div className={`backdrop-blur-md border rounded-2xl p-4 shadow-2xl animate-in slide-in-from-top-2 duration-500 ${
          isWarning
          ...
          <div className="absolute inset-0 opacity-10">
            ...
            <div className={`absolute bottom-2 left-2 w-4 h-4 rounded-full animate-bounce delay-300 ${
          ...
          <div className="relative z-10">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className={`text-sm flex items-center space-x-2 ${
                    ...
                    <span>{isWarning ? 'Action required' : 'Execution failed'}</span>
                  </div>
                </div>
              </div>
            ...
            {/* Expanded Details */}
            {showDetails && (
              <div className={`mt-3 pt-3 border-t text-xs space-y-1 animate-in fade-in slide-in-from-top-1 duration-200 ${
                isWarning ? 'border-yellow-400/20 text-yellow-200' : 'border-red-400/20 text-red-200'
              }`}>
                <div>Request ID: {error.requestId}</div>
                <div>Timestamp: {new Date(error.timestamp).toLocaleTimeString()}</div>
              </div>
            )}
          </div>
        </div>
