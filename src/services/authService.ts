...
class ApiClient {
      ...
      if (response.status === 401 && !endpoint.includes('/auth/refresh-token')) {
        ...
        if (errorData.code === 'ACCESS_TOKEN_EXPIRED') {
          ...
          if (!retryResponse.ok) {
            ...
            throw new Error(retryError.message || 'Request failed after token refresh');
          }
  ...
  async verifyToken(): Promise<{ valid: boolean; user?: any }> {
    try {
      return await this.makeRequest<{ valid: boolean; user: any }>('/auth/verify-token');
    } catch (error) {
      return { valid: false };
    }
  }
...
const authService = {
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    ...
    apiClient.storeAuthData(response.accessToken, response.refreshToken, response.user);
    return response;
  },
  ...
  storeAuthData(_token: string, _user: AuthResponse['user']): void {
    // For backward compatibility, assume this is access token only
    // In practice, this should be updated to use the new method
    console.warn('authService.storeAuthData is deprecated. Use the new format with refresh tokens.');
  },
